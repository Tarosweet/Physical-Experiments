...
public class DownTransferFluidActionBuilder : IFluidActionBuilder
{
    private FluidContainer _containerFrom;
    private FluidContainer _containerTo;
    private float _count;
    
    public DownTransferFluidActionBuilder(FluidContainer containerFrom, FluidContainer containerTo, float count)
    {
    ...
    public IFluidAction Build()
    {
        ...
        for (int i = 0; i < fluidsFrom.Count; i++)
        {
    ...
    private IFluidAction Transfer(FluidContainer container, List<Fluid> fluids, List<float> counts)
    {
        ...
        if (diff >= 0)
        {
            ...
            while (currDiff > 0)
            {
                ...
                {
                    action = new DecreaseFluidAction(container,tmpFluid,count);
